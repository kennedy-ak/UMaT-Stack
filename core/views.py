...
def index(request):
    ...
    users_following = FollowersCount.objects.filter(follower=request.user.username)
    for users in users_following:
...
def profile(request,slug):
    ...
    user_profile = Profile.objects.get(user=user_object)
    user_posts = Post.objects.filter(user=slug)
    ...
    users_followers = len(FollowersCount.objects.filter(user=slug))
    users_following = len(FollowersCount.objects.filter(user=slug))
